    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"⚠️ 𝐸𝑅𝑅𝒪𝑅 ⚠️ DM --> @VIP_DDoS_SELLER"
        else:
            response = "⚠️ 𝐸𝑅𝑅𝒪𝑅 ⚠️ DM --> @VIP_DDoS_SELLER"
    else:
        response = f"⚠️ 𝐸𝑅𝑅𝒪𝑅 ⚠️ DM --> @VIP_DDoS_SELLER"
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users or datetime.datetime.now() > datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S'):
        await update.message.reply_text("𝗬𝗼𝘂 𝗮𝗿𝗲 𝗻𝗼𝘁 𝗮𝘂𝘁𝗵𝗼𝗿𝗶𝘀𝗲𝗱 𝗯𝘆 𝗦𝟰 𝗢𝗙𝗙𝗜𝗖𝗜𝗔𝗟🚩 𝗽𝗹𝗲𝗮𝘀𝗲 𝗰𝗼𝗻𝘁𝗮𝗰𝘁 [ @VIP_DDoS_SELLER ]")
        return
